...
import { Box, Table, Thead, Tbody, Tr, Th, Td, Spinner, Input, Button, Flex, IconButton } from '@chakra-ui/react';
...
const sampleSalesData = [
    {
      ...
      },
      {
        ...
        orderItems: [
          { productId: 'p7', quantity: 1, price: 50 }
        ]
      },
];
...
const RecentSalesTable: React.FC = () => {
  ...
  const handleSearch = () => {
    const filtered = sampleSalesData.filter(sale => sale.userId.toLowerCase().includes(searchUserId.toLowerCase()));
    setSales(filtered);
